...
namespace detail {
  ...
  template<typename T> using describe_members_t = boost::describe::describe_members<T, boost::describe::mod_any_access>;
  ...
  template<typename T> auto member_byte_offset(auto member_pointer) -> std::int64_t
  {
  ...
  template<auto MemberPtr> struct matches_member_pointer
  {
    template<typename Descriptor> using fn = std::bool_constant<Descriptor::pointer == MemberPtr>;
  };
...
/// ```
template<typename T> struct push_constant
{
  ...
  [[nodiscard]] static auto glsl_block() -> std::string
  {
    ...
    boost::mp11::mp_for_each<detail::describe_members_t<T>>([&](auto descriptor) -> void {
      using member_type = std::remove_cv_t<std::remove_reference_t<decltype(std::declval<T>().*descriptor.pointer)>>;
      body += "  ";
  ...
  template<auto MemberPtr> [[nodiscard]] auto get() const
  {
