...
inline auto cmd_push_data(context const &ctx, VkCommandBuffer cmd, std::span<std::byte const> bytes, std::uint32_t offset = 0)
  -> void
{
  if (ctx.procs().cmd_push_data == nullptr) {
    VKEXEC_THROW(std::runtime_error("vkCmdPushDataEXT is unavailable"));
  }
  if (bytes.empty()) { VKEXEC_THROW(std::invalid_argument("cmd_push_data requires a non-empty payload")); }
...
template<typename T>
auto cmd_push_data(context const &ctx, VkCommandBuffer cmd, T const &value, std::uint32_t offset = 0) -> void
{
  static_assert(std::is_trivially_copyable_v<T>);
  // NOLINTNEXTLINE(cppcoreguidelines-pro-type-reinterpret-cast)
  auto const *const bytes = reinterpret_cast<std::byte const *>(&value);
  cmd_push_data(ctx, cmd, std::span<std::byte const>{ bytes, sizeof(T) }, offset);
}
