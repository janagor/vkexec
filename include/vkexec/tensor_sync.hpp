    ...
    return pass_step{
      .record =
        [target](context & /*ctx*/, VkCommandBuffer cmd, pass_cleanup & /*cleanup*/) -> status {
          ...
          device_barrier.srcAccessMask = static_cast<VkAccessFlags>(
            static_cast<std::uint32_t>(VK_ACCESS_SHADER_WRITE_BIT) | static_cast<std::uint32_t>(VK_ACCESS_TRANSFER_WRITE_BIT));
          ...
          // NOLINTBEGIN(hicpp-signed-bitwise)
          auto const src_stage = static_cast<VkPipelineStageFlags>(
...
[[nodiscard]] auto
  lower_vkexec_sender(ex::set_value_t /*tag*/, pass_adaptor_sender<Pred, sync_to_device_closure<T>> sndr, Env const & /*env*/)
{
  ...
  return ex::let_value(
    std::move(sndr.pred), [ctx, closure = sndr.closure](auto &&...) mutable -> pass_graph_sender {
...
template<class Pred, typename T, class Env>
[[nodiscard]] auto
  lower_vkexec_sender(ex::set_value_t /*tag*/, pass_adaptor_sender<Pred, sync_to_host_closure<T>> sndr, Env const & /*env*/)
{
  scheduler const sched = ex::get_completion_scheduler<ex::set_value_t>(ex::get_env(sndr.pred));
  // NOLINTNEXTLINE(misc-const-correctness)
  context *const ctx = sched.get_context();
