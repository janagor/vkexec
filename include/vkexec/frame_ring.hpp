...
public:
  ...
  auto resize_images(std::size_t image_count) -> detail::status;
  ...
  [[nodiscard]] auto acquire_semaphore(std::size_t slot) const -> detail::result<VkSemaphore>;
  [[nodiscard]] auto render_finished_semaphore(std::size_t image_index) const -> detail::result<VkSemaphore>;
  ...
  [[nodiscard]] auto wait_slot(std::size_t slot) const -> detail::status;

  /// Host-wait until the GPU finished the previous submit that used this image.
  [[nodiscard]] auto wait_image(std::size_t image_index) const -> detail::status;
  ...
  auto mark_submitted(std::size_t slot, std::size_t image_index, std::uint64_t signal_value) -> detail::status;
  ...
  [[nodiscard]] auto make_submit_sync(std::size_t slot,
    ...
    -> detail::result<frame_ring_submit_sync>;
...
private:
  ...
  auto create_image_semaphores(std::size_t image_count) -> detail::status;
  [[nodiscard]] auto check_slot(std::size_t slot) const -> detail::status;
  [[nodiscard]] auto check_image(std::size_t image_index) const -> detail::status;
