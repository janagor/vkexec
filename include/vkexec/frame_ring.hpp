...
public:
  ...
  [[nodiscard]] static auto create(context &ctx, create_info info) -> frame_ring;
  ...
  auto resize_images(std::size_t image_count) -> void;
  ...
  [[nodiscard]] auto acquire_semaphore(std::size_t slot) const -> VkSemaphore;
  [[nodiscard]] auto render_finished_semaphore(std::size_t image_index) const -> VkSemaphore;
  ...
  auto wait_slot(std::size_t slot) const -> void;

  /// Host-wait until the GPU finished the previous submit that used this image.
  auto wait_image(std::size_t image_index) const -> void;
  ...
  auto mark_submitted(std::size_t slot, std::size_t image_index, std::uint64_t signal_value) -> void;
  ...
  [[nodiscard]] auto make_submit_sync(std::size_t slot,
    ...
    -> frame_ring_submit_sync;

private:
  frame_ring(context *ctx, create_info info);

  auto destroy() noexcept -> void;
  auto destroy_image_semaphores() noexcept -> void;
  auto create_image_semaphores(std::size_t image_count) -> void;
  auto check_slot(std::size_t slot) const -> void;
  auto check_image(std::size_t image_index) const -> void;
