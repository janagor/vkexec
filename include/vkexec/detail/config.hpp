...
[[noreturn]] inline auto terminate() noexcept -> void
{
  std::terminate();
}
...
#  define VKEXEC_CATCH(...) } else if constexpr (__VA_ARGS__ = ::vkexec::detail::k_catch_any_lvalue; false) {
#  define VKEXEC_CATCH_ALL } else if constexpr (true) {} else
#  define VKEXEC_THROW(...) ::vkexec::detail::terminate()
#  define VKEXEC_CATCH_FALLTHROUGH } else {}
#else
