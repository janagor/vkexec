...
template<typename Params>
auto compute_heap_pass(compute_pipeline const &pipe, Params const &params, std::uint32_t work_count)
  -> heap_compute_pass_closure
{ return compute_heap_pass(pipe.bind(), params, pipe.groups_for(work_count)); }

auto compute_heap_pass(compute_pipeline const &pipe, std::uint32_t work_count) -> heap_compute_pass_closure;

template<typename Params>
auto compute_heap_pass(compute_pipeline const &pipe, Params const &params, indirect_dispatch groups)
  -> heap_compute_pass_closure
{ return compute_heap_pass(pipe.bind(), params, groups); }

auto compute_heap_pass(compute_pipeline const &pipe, indirect_dispatch groups) -> heap_compute_pass_closure;
