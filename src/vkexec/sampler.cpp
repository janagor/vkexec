...
{
  if (ctx.device() == VK_NULL_HANDLE) { return make_error(errc::invalid_argument, "sampler requires a VkDevice"); }
  ...
  if (create_result != VK_SUCCESS) { return make_vk_error(create_result, "vkCreateSampler failed"); }
