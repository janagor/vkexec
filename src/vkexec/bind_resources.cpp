...
auto bind_resources(pipeline_resources const &pipe,
...
{
  auto step = detail::make_bind_resources_step<detail::set_descriptor_backend>(closure.pipe,
    std::move(closure.table),
    detail::empty_table_lower_env{},
    std::move(closure.push));
...
auto operator|(pass_graph_sender graph, bind_resources_closure closure) -> pass_graph_sender
{
  return detail::append_step(std::move(graph),
    detail::make_bind_resources_step<detail::set_descriptor_backend>(closure.pipe,
      std::move(closure.table),
      detail::empty_table_lower_env{},
      std::move(closure.push)));
}
