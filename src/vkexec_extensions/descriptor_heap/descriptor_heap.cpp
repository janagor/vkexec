...
namespace {
  ...
  // NOLINTEND(bugprone-easily-swappable-parameters)
  {
    ...
    if (destination.empty()) { return fail(errc::invalid_argument, empty_message); }
...
auto query_descriptor_heap_layout(context const &ctx) -> result<descriptor_heap_layout>
{
  ...
  if (layout.sampler_descriptor_size == 0) {
    return fail(errc::unsupported, "descriptor heap properties reported a zero sampler descriptor size");
  }
...
// NOLINTEND(bugprone-easily-swappable-parameters)
{
  ...
  VkResult const write_result = procs.write_sampler_descriptors(ctx.device(), 1, &sampler_info, &host_range);
  if (write_result != VK_SUCCESS) { return fail(write_result, "vkWriteSamplerDescriptorsEXT failed"); }
  return {};
}
...
// NOLINTEND(bugprone-easily-swappable-parameters)
{
  ...
  if (procs.cmd_bind_sampler_heap == nullptr) {
    return fail(errc::unsupported, "vkCmdBindSamplerHeapEXT is unavailable");
  }
  ...
  bind_info.reservedRangeOffset = reserved_range_offset;
  bind_info.reservedRangeSize = reserved_range_size;
  procs.cmd_bind_sampler_heap(cmd, &bind_info);
  return {};
}
