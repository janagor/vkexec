...
TEST_CASE("ext::configure enables dynamic_rendering on Vulkan 1.3+", "[vkexec][extension][gpu]")
{
  vkexec::vulkan_requirements requirements{};
  requirements.api_version_major = 1;
  ...
  auto ctx = vkexec::test::sync_wait_value(vkexec::context::create({ .requirements = std::move(requirements) }));
  REQUIRE(vkexec::ext::available<vkexec::ext::dynamic_rendering>(*ctx));
  REQUIRE(vkexec::ext::name<vkexec::ext::dynamic_rendering>() == "dynamic_rendering");
}
