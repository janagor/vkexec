...
static auto run() -> int
{
  ...
  auto input = vkexec::examples::sync_wait_value(vkexec::factory::make_buffer(*ctx, k_element_count, k_initial));
  auto output = vkexec::examples::sync_wait_value(vkexec::factory::make_buffer(*ctx, k_element_count, 0.0F));
